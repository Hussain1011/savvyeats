...
class BOM(WebsiteGenerator):
	...
	def update_production_item(self):
		NUTRIENT_KEYS = ["Calories", "Protein", "Fats", "Net Carbs", "Fibers"]

		def get_item_nutrients(item_code, qty):
	...
	def set_bom_in_raw_materials(self):
		for d in self.items:
			item = frappe.get_doc("Item", d.item_code)
			if item.item_category not in ("Ingredient", "Sub Recipe"):
				frappe.throw(_("Only Item Category Ingredient and Sub Recipe are allowed in Raw Materials"))
			if item.item_category == "Sub Recipe" and not d.bom_no:
				boms = frappe.get_all("BOM", filters={"item": d.item_code, "is_active": 1, "is_default": 1})
				if not boms:
					frappe.throw(_("No Active and Default BOM found for Item  {0}:{1} in row: {2}".format(d.item_code, d.item_name, d.idx)))

				d.bom_no = boms[0].name
